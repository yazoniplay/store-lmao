...
async function initBackend(){
  try{
    ...
    await loadScript("velora-config.js");
    await loadScript("velora-backend.js");
...
function add(id,qty=1){
 const p=activeProducts().find(x=>Number(x.id)===Number(id));if(!p)return;
 const current=cart.find(x=>Number(x.id)===Number(id)),next=(current?.quantity||0)+qty;
 if(p.stock!==undefined&&p.stock!==""&&next>Number(p.stock)){toast("Not enough stock available.");return}
 current?current.quantity=next:cart.push({id:Number(p.id),quantity:qty});saveCart();openCart();toast("Added to bag");
}
function changeQty(id,delta){
 const item=cart.find(x=>Number(x.id)===Number(id)),p=activeProducts().find(x=>Number(x.id)===Number(id));if(!item)return;
 const next=item.quantity+delta;if(next<=0){removeItem(id);return}
...
function renderCart(){
 const items=document.getElementById("cartItems"),count=document.getElementById("cartCount"),total=document.getElementById("cartTotal"),sub=document.getElementById("cartSubtotal"),ship=document.getElementById("cartShipping");if(!items)return;
 cart=cart.filter(x=>products.some(p=>Number(p.id)===Number(x.id)&&p.status!=="Draft"));
 const qty=cart.reduce((s,x)=>s+x.quantity,0);if(count){count.textContent=qty;count.classList.toggle("bump",qty>0)}
 const subtotal=cartSubtotal(),shipCost=shipping();if(sub)sub.textContent=money(subtotal);if(ship)ship.textContent=shipCost===0?"Free":money(shipCost);if(total)total.textContent=money(subtotal+shipCost);
 items.innerHTML=cart.length?cart.map(x=>{const p=products.find(y=>Number(y.id)===Number(x.id));return '<div class="cart-item"><img src="'+(p.image||"")+'" alt="'+esc(p.name)+'"><div class="cart-item-main"><strong>'+esc(p.name)+'</strong><span>'+money(p.price)+'</span><div class="qty-control"><button data-minus="'+p.id+'">−</button><b>'+x.quantity+'</b><button data-plus="'+p.id+'">+</button></div><button class="remove-item" data-remove="'+p.id+'">Remove</button></div><strong>'+money(p.price*x.quantity)+'</strong></div>"}).join(""):'<div class="cart-empty"><strong>Your bag is empty.</strong><span>Add something useful.</span></div>';
 ...
 document.querySelectorAll("[data-remove]").forEach(b=>b.onclick=()=>removeItem(Number(b.dataset.remove)));localStorage.setItem(CART_KEY,JSON.stringify(cart));
}
...
async function checkout(){
 ...
 if(backend){try{toast("Opening secure checkout…");await window.VeloraBackend.checkout(cart)}catch(e){console.error(e);toast(e.message||"Checkout failed.")};return}
 let m=document.getElementById("paymentProviderModal");if(!m){m=document.createElement("div");m.id="paymentProviderModal";m.className="payment-modal";m.innerHTML='<div class="payment-modal-backdrop" data-close-pay></div><div class="payment-dialog"><button class="payment-close" data-close-pay>×</button><p class="payment-eyebrow">VELORA / CHECKOUT</p><h2>Backend not connected.</h2><p>Add the Supabase project settings to enable live Stripe checkout.</p><button class="button button-dark full" data-close-pay>Close</button></div>';document.body.appendChild(m);m.querySelectorAll("[data-close-pay]").forEach(x=>x.onclick=()=>m.classList.remove("open"))}m.classList.add("open")
}
...
function setupCatalog(){
 ...
 const tabs=document.querySelectorAll("[data-filter]");let filter="all";
 const categories=[...new Set(activeProducts().map(p=>p.category).filter(Boolean))],wrap=document.getElementById("categoryFilters");
 ...
 function apply(){
  const q=(input.value||"").trim().toLowerCase();let list=activeProducts();
  if(filter==="new")list=list.filter(p=>p.badge==="New"||p.isNew);else if(filter==="best")list=list.filter(p=>p.badge==="Best Seller"||p.bestSeller);else if(filter!=="all")list=list.filter(p=>p.category===filter);
  if(q)list=list.filter(p=>(p.name+" "+(p.description||"")+" "+(p.category||"")).toLowerCase().includes(q));renderProducts(list);
 ...
 document.getElementById("searchToggle")?.addEventListener("click",()=>input.focus());
 input.addEventListener("input",apply);apply();
}
function setupProduct(){
 const root=document.getElementById("productDetail");if(!root)return;
 const id=Number(new URLSearchParams(location.search).get("id")),p=activeProducts().find(x=>Number(x.id)===id);
 ...
 const images=(p.images&&p.images.length?p.images:[p.image]).filter(Boolean),related=activeProducts().filter(x=>x.id!==p.id&&(x.category===p.category||x.badge==="Best Seller")).slice(0,4);
...
async function boot(){
 ...
 if(isHome){const featured=activeProducts().filter(p=>p.featured||p.badge==="Featured").slice(0,3);renderProducts(featured.length?featured:activeProducts().slice(0,3))}
 setupGlobal();setupCatalog();setupProduct();
 if(location.hash==="#search")setTimeout(()=>document.getElementById("productSearch")?.focus(),100);
}
boot();
